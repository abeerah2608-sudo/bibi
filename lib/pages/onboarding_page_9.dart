...
class _OnboardingPage9State extends State<OnboardingPage9>
    with TickerProviderStateMixin, OnboardingAudioMixin<OnboardingPage9>, WidgetsBindingObserver, RouteAware {
  static const String _targetPageId = 'page_12';
  ...
  OnboardingPageData? _findTargetPage(List<OnboardingPageData> pages) {
    for (final page in pages) {
      if (page.id == _targetPageId ||
          page.pageType == 'food_items_grid' ||
          page.textKey == 'how_to_prevent_title') {
        return page;
      }
    }
    return null;
  ...
  Future<void> _primeAssets(OnboardingPageData page) async {
    try {
      ...
      debugPrint('⚠️ Page9 asset priming failed: $e');
    }
  }
  ...
  void _hydrateFromLoaded(OnboardingLoaded state) {
    ...
    if (found == null) {
      debugPrint('⚠️ Page9 could not find page_12 / food_items_grid');
      return;
    }
    ...
    debugPrint('✅ Page9 hydrated from Firestore: id=${found.id}, type=${found.pageType}, textKey=${found.textKey}');
  }
  ...
  Widget _buildGrid(BuildContext context, OnboardingPageData page, String currentLanguage) {
    ...
    final titleHorizontalPadding = (titleStyle.titleHorizontalPadding ?? 4).w;
                  ...
                  onBackPressed: () {
                    ...
                    if (Navigator.of(context).canPop()) {
                      ...
                      Navigator.of(context).push(
                        MaterialPageRoute(builder: (_) => const OnboardingPage8()),
                      );
                    }
                  },
                  onNextPressed: () {
